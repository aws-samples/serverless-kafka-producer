...
class ServerlessKafkaConsumerStack(Stack):
    ...
    def init_kafka_consumer_lambda(
        ...
        consumer_security_group = ec2.SecurityGroup(self, 'KafkaConsumerSecurityGroup', allow_all_outbound=kafka_security_group.allow_all_outbound, disable_inline_rules=kafka_security_group.can_inline_rule, vpc=vpc)

        consumer_security_group.connections.allow_from(kafka_security_group, ec2.Port.tcp(9098), 'Allow port 9098 from MSK Serverless cluster security group')
        consumer_security_group.connections.allow_from(kafka_security_group, ec2.Port.tcp(2182), 'Allow port 2182 from MSK Serverless cluster security group')
        consumer_security_group.connections.allow_from(kafka_security_group, ec2.Port.tcp(2181), 'Allow port 2181 from MSK Serverless cluster security group')  
        ...
        consumer_function = f.Function(
            ...
            security_groups=[consumer_security_group],
